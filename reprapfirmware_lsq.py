...
class Tuner(object):
    def __init__(self, old_rod_length, old_radius, old_homed_height, old_xstop, old_ystop, old_zstop,
                 old_xpos, old_ypos, old_zpos, probe_radius=64, num_probe_points=7, num_factors=6, base_height=185.1):
    ...
    def _generate_commands(self):
        ...
        elif self.new_params.firmware in [DeltaParameters.FIRMWARE_MARLIN_RICH_CATTEL]:
        ...
        if self.new_params.firmware == DeltaParameters.FIRMWARE_MARLIN:
            m_665 += " A{:.2f} B{:.2f} C{:.2f}".format(self.new_params.xadj,
                    self.new_params.yadj,
                    self.new_params.zadj)
            m_206 = "M206 Z{:.2f}".format(self.new_params.homed_height - self.base_height)
        commands = (m_665, m_666, m_206)
        return commands
